...
void CreatureType::randomizeTypes() {
	...
	ofstream fout;
	fout.open("creatureInfo.txt");
	for (int i = 0; i < NUM_TYPES; i++){
		fout << "Name: "          << CREATURE_NAMES[i] << endl;
		fout << "Max Health: "    << TYPES[i].healthMaxBase << endl;
		fout << "Rest Increase: " << TYPES[i].healthRestIncrease << endl;
		fout << "Attack Base: "   << TYPES[i].attackBase << endl;
		fout << "Attack Type: "   << ELEMENT_NAMES[TYPES[i].elementalAttackType] << endl;
		fout << "Weakness: "      << ELEMENT_NAMES[TYPES[i].elementalWeakness] << endl;
		fout << "Strength: "      << ELEMENT_NAMES[TYPES[i].elementalStrength] << endl;
		fout << endl;
	}
	fout.close();
}
